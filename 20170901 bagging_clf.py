...
def setDict_train_validation(originDataFile):
    if os.path.exists(filePath):
        ...
        dataMatrix = np.array(pd.read_csv(filePath, header=None, skiprows=1, names=listTitle))
...
def main():
    ...
    # 原始数据的csv文件
    originDataFile = 'F:\Code\classify test\\jupyter-test.csv'  #注意：该csv文件没有'ID'列
    # 生成的训练集csv文件和验证集csv文件
    trainDataFile = 'F:\Code\classify test\\jupyter-train.csv'  #注意：该csv文件没有'ID'列
    validationDataFile = 'F:\Code\classify test\\jupyter-validation.csv'  #注意：该csv文件没有'ID'列
