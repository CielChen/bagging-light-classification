...
Author: CIEL
Function:
    更换了新的数据集，重新训练基于GBDT（梯度提升树）的分类器
'''
...
def generateDataDict(dataFile):
    if os.path.exists(dataFile):
        ...
        featureNum = 342  # 20171114BalanceTrain.csv，共有342维特征（像素位置+超像素）
...
def trainModel(X_train, y_train, X_test, y_test):
    ...
    # model = gbm0.fit(X_train, y_train)
    ...
    param_test1 = {'n_estimators': list(range(900, 2001, 100))}
    gsearch1 = GridSearchCV(estimator=GradientBoostingClassifier(learning_rate=0.1, min_samples_split=60,
                                                                 ...
                                                                 random_state=10),
                            param_grid=param_test1, scoring='roc_auc', iid=False, cv=10, n_jobs=-1, verbose=2)
    ...
    # # --------------- step6: 最大特征数max_features -------------------------
    # # 上一步找到了最优的迭代次数n_estimators=1000，min_samples_leaf=20, min_samples_split=100，max_depth=11
    # print('GridSearching ...')
    # # param_test5 = {'max_features': list(range(18, 101, 2))}      # 共有342维特征，sqrt(342)约等于18
    # param_test5 = {'max_features': list(range(100, 201, 2))}  # 共有342维特征，sqrt(342)约等于18
    # gsearch5 = GridSearchCV(estimator=GradientBoostingClassifier(learning_rate=0.1, n_estimators=1000,
    #                                                              max_depth=11, min_samples_leaf=20,
    #                                                              min_samples_split=100, subsample=0.8,
    #                                                              random_state=10),
    #                         param_grid=param_test5, scoring='roc_auc', iid=False, cv=5, n_jobs=-1, verbose=2)
    # clf = gsearch5.fit(X_train, y_train)
    # print('GridSearch Result:')
    # print(gsearch5.grid_scores_, gsearch5.best_params_, gsearch5.best_score_)

    # # --------------- step7: 子采样的比例subsample -------------------------
    # # 上一步找到了最优的迭代次数n_estimators=1000，min_samples_leaf=20, min_samples_split=100，max_depth=11
    # # 上一步找到了最优的最大特征数max_features=313
    # print('GridSearching ...')
    # # param_test6 = {'subsample': floatRange(0.4, 1, 13)}  # 0.4, 0.45, 0.5, 0.55, ···, 1.0
    # # param_test6 = {'subsample': floatRange(0.8, 1, 21)}  # 0.8, 0.81, 0.82, 0.83, ···, 1.0
    # param_test6 = {'subsample': floatRange(0.92, 0.94, 21)}  # 0.92, 0.921, 0.922, 0.923, ···, 0.94
    # gsearch6 = GridSearchCV(estimator=GradientBoostingClassifier(learning_rate=0.1, n_estimators=1000,
    #                                                              max_depth=11, min_samples_leaf=20,
    #                                                              min_samples_split=100, max_features=313,
    #                                                              random_state=10),
    #                         param_grid=param_test6, scoring='roc_auc', iid=False, cv=5, n_jobs=-1, verbose=2)
    # clf = gsearch6.fit(X_train, y_train)
    # print('GridSearch Result:')
    ...
    # # --------------- step8:  步长learning_rate -------------------------
    # # 上一步找到了最优的迭代次数n_estimators=1000，min_samples_leaf=20, min_samples_split=100，max_depth=11
    # # 上一步找到了最优的最大特征数max_features=313
    # # 上一步找到了最优的子采样的比例subsample=0.93
    # print('GridSearching ...')
    # # 之前的learning_rate=0.1, n_estimators=1000. 现在逐步调节learning_rate
    # # //////////////////////// case 1: learning_rate降一半，即learning_rate=0.05, 相应的n_estimators加倍，即n_estimators=2000
    # # model = GradientBoostingClassifier(learning_rate=0.05, n_estimators=2000, max_depth=11,
    # #                                    min_samples_leaf=20, min_samples_split=100, max_features=313,
    # #                                    subsample=0.93, random_state=10)
    # # //////////////////////// case 2: learning_rate=0.01, n_estimators=10000
    # model = GradientBoostingClassifier(learning_rate=0.01, n_estimators=10000, max_depth=11,
    #                                    min_samples_leaf=20, min_samples_split=100, max_features=313,
    #                                    subsample=0.93, random_state=10)
...
def lightSource():
    ...
    outputFile = open("lightOnly.txt", 'w')
    with open('modelResult.txt', 'r') as modelResult:
        for line in modelResult:
            if(line[0]=='1'):
...
def main():
    ...
    dataFile = '20171114BalanceTrain.csv'  # 注意：该csv文件没有'ID'列
    print('生成数据字典')
    # 生成训练集数据字典
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20, random_state=42)   # 随机分成两部分，训练集占80%，测试集占20%
    # print("trainDict")
    # print("feature")
    # print (X_train)
    # print("class")
