...
def trainModel(X_train, y_train, X_test, y_test):
    ...
    param_test1 = {'n_estimators': list(range(20, 100, 10))}
    ...
    # print('GridSearching ...')
    # # param_test5 = {'max_features': list(range(1, 22, 2))}      # 共有342维特征，sqrt(342)约等于18
    # param_test5 = {'max_features': list(range(100, 201, 1))}  # 共有342维特征，sqrt(342)约等于18
...
# 部分调参过程的可视化，即不同参数下的结果比较
def strategyGBDT(X_train, y_train, X_test, y_test):
    print('strategy result ...')
    original_params = {'n_estimators': 100, 'max_leaf_nodes': 4, 'max_depth': 11, 'random_state': 10,
                       'min_samples_split': 60}
    plt.figure()
    for label, color, setting in [('No shrinkage', 'orange', {'learning_rate':1.0, 'subsample':1.0}),
                                  ('learning_rate=1.0', 'turquoise', {'learning_rate':0.1, 'subsample':1.0}),
                                  ('subsample=0.5', 'blue', {'learning_rate':1.0, 'subsample':0.5}),
                                  ('learning_rate=0.1, subsample=0.5', 'gray', {'learning_rate':0.1, 'subsample':0.5}),
                                  ('learning_rate=0.1, max_features=1', 'magenta', {'learning_rate':0.1, 'max_features':2})]:
...
def main():
    ...
    # -------------------------------- step 7. 部分调参过程的可视化 --------------------------------
    # 运行本程序的前提是已经知道各个参数的最优值
    strategyGBDT(X_train, y_train, X_test, y_test)
